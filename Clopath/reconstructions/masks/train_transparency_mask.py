...
import Clopath.reconstructions.masks.utils_reconstruction as utils
...
def train_mask(model_path, mouse_indexes, datafold, skip_frames=50, length=32):

    print(f'\nTraining transparency mask of model {model_path}, on {datafold}')

    model = argus.load_model(model_path, device="cuda:1", optimizer=None, loss=None)
    ...
    for mouse_index in mouse_indexes:
        ...
        mouse_data = get_mouse_data(mouse=mouse, splits=[datafold], s_type='movie')
...
if __name__ == '__main__':
    ...
    model_path = Path('/home/albertestop/Sensorium/data/experiments/train_test_018_3/fold_0/model-025-0.100987.pth')
    mouse_indexes = [0]
    ...
    train_mask(model_path, mouse_indexes, datafold, skip_frames, length)