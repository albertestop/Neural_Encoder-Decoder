...
json_file_path = Path(parent_dir / Path('Clopath/folds_trials_new_mice.json'))
...
for mouse_index in animals:
    ...
    for trial in trials_to_process:
        ...
        if population_reduction > 0:
            manual_seed = (trial + 1) * (mouse_index + 1)
            print(f'Random seed for population reduction: {manual_seed}')
            torch.manual_seed(manual_seed)
        ...
        fig.suptitle(f'mouse {mouse_key} trial {trial} model {model_list}', fontsize=16)
        ...
        axs[2, 1].imshow(responses_predicted_mean.cpu().detach().numpy(), aspect='auto', vmin=0, vmax=10)
        axs[2, 1].set_title('Predicted responses')
        axs[3, 0].imshow(responses_predicted_gray.mean(axis=2).cpu().detach().numpy(), aspect='auto', vmin=0, vmax=10)
        ...
        fig.savefig(save_path + f'reconstruction_summary_m{mouse_key}_t{trial}{randneurons_suffix}.png')
        ...
        print(f'Reconstructing mouse {mouse_key} trial {trial}')
        ...
        for i in progress_bar:
            ...
            if i == 0 or i % track_iter == 0 or i == epoch_switch[-1] - 1:
                ...
                concat_video = utils.save_tif(
                    ground_truth,
                    reconstruction,
                    save_path + f'optimized_input_m{mouse_key}_t{trial}_models_{model_list_str}{randneurons_suffix}.tif',
                    mask=mask_cropped
                )
                ...
                if i == 0 or i % plot_iter == 0 or i == epoch_switch[-1] - 1:
                    ...
                    axs[3, 1].clear()
                    axs[3, 1].imshow(responses_predicted_full.cpu().detach().numpy(), aspect='auto', vmin=0, vmax=10)
        ...
        np.save(save_path + f'reconstruction_summary_m{mouse_key}_t{trial}{randneurons_suffix}.npy', recon_dict)
        ...
        tif_path = Path(save_path) / f'optimized_input_m{mouse_key}_t{trial}_models_{model_list_str}{randneurons_suffix}.tif'
        mp4_path = Path(save_path) / f'reconstruction_video_m{mouse_key}_t{trial}_models_{model_list_str}{randneurons_suffix}.mp4'
