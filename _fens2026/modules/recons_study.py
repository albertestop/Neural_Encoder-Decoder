from pathlib import Path
import os
import sys
import numpy as np

current_dir = Path(__file__).resolve().parent
parent_dir = current_dir.parent.parent
sys.path.append(str(parent_dir))

from _fens2026.src.str_metrics import *
from _fens2026.src.str_metrics_new import *
from _fens2026.src.data_loading import *


def fill_edge_zeros_with_mean(arr):
    if arr.size == 0:
        return arr

    nonzero_idx = np.flatnonzero(arr != 0)
    if nonzero_idx.size == 0:
        return arr

    mean_val = np.mean(arr)
    first_nz = nonzero_idx[0]
    last_nz = nonzero_idx[-1]

    arr[:first_nz] = mean_val
    arr[last_nz + 1:] = mean_val
    return arr


def compute_recons_metrics(session, recons_n, metric_window_t):
    window = metric_window_t * 30
    proc_config_path = "/home/albertestop/data/processed_data/sensorium_all_2023/" + session + "/config.py"
    proc_config = load_config(proc_config_path)
    recons_path = Path(proc_config.exp_directory + proc_config.animal + '/' + proc_config.session + '/reconstructions/' + recons_n + '/' + session + '/reconstruction/')
    recons_config_path = str(recons_path) + '/' + os.listdir(recons_path)[0] + '/config.py'
    rec_config = load_config(recons_config_path)

    mask = load_mask(rec_config, proc_config, session)
    mask = np.where(mask >= rec_config.mask_eval_th, 1, 0)
    mask = mask == 1
    mask = cut_exp_mask(mask, rec_config.mask_eval_th, window)
    recons = np.load(str(recons_path.parent) + '/whole_session_recons/video_array.npy')
    recons_time = np.load(str(recons_path.parent) + '/whole_session_recons/video_timeline.npy')

    temporal_corr_evo = np.zeros(recons_time.shape)
    temporal_ssim_evo = np.zeros(recons_time.shape)
    spectral_slope_evo = np.zeros(recons_time.shape)
    compression_gain_evo = np.zeros(recons_time.shape)
    video_comp_gain_evo = np.zeros(recons_time.shape)

    print("Computing metrics")
    for i in range(len(recons_time) - window):
        print(f"{((i/len(recons_time)) * 100):.3f}" + "%")
        window_recons = recons[i:i + window]
        temporal_corr_evo[i + int(window / 2)] = temporal_corr(window_recons, mask)
        temporal_ssim_evo[i + int(window / 2)] = temporal_ssim(window_recons, mask)
        spectral_slope_evo[i + int(window / 2)] = spectral_slope(window_recons, mask)
        compression_gain_evo[i + int(window / 2)] = compression_gain(window_recons, mask)
        video_comp_gain_evo[i + int(window / 2)] = video_comp_gain(window_recons, mask)

    temporal_corr_evo = fill_edge_zeros_with_mean(temporal_corr_evo)
    temporal_ssim_evo = fill_edge_zeros_with_mean(temporal_ssim_evo)
    spectral_slope_evo = fill_edge_zeros_with_mean(spectral_slope_evo)
    compression_gain_evo = fill_edge_zeros_with_mean(compression_gain_evo)
    video_comp_gain_evo = fill_edge_zeros_with_mean(video_comp_gain_evo)

    temporal_corr_evo = np.transpose(np.vstack((recons_time, temporal_corr_evo)))
    temporal_ssim_evo = np.transpose(np.vstack((recons_time, temporal_ssim_evo)))
    spectral_slope_evo = np.transpose(np.vstack((recons_time, spectral_slope_evo)))
    compression_gain_evo = np.transpose(np.vstack((recons_time, compression_gain_evo)))
    video_comp_gain_evo = np.transpose(np.vstack((recons_time, video_comp_gain_evo)))

    save_path = recons_path.parent / Path('metrics')
    os.makedirs(save_path, exist_ok=True)
    np.save(str(save_path) + '/temporal_corr.npy', temporal_corr_evo)
    np.save(str(save_path) + '/temporal_ssim.npy', temporal_ssim_evo)
    np.save(str(save_path) + '/spectral_slope.npy', spectral_slope_evo)
    np.save(str(save_path) + '/compression_gain.npy', compression_gain_evo)
    np.save(str(save_path) + '/video_comp_gain_evo.npy', video_comp_gain_evo)


def compute_new_recons_metrics(session, recons_n, metric_window_t):
    window = metric_window_t * 30
    proc_config_path = "/home/albertestop/data/processed_data/sensorium_all_2023/" + session + "/config.py"
    proc_config = load_config(proc_config_path)
    recons_path = Path(proc_config.exp_directory + proc_config.animal + '/' + proc_config.session + '/reconstructions/' + recons_n + '/' + session + '/reconstruction/')
    recons_config_path = str(recons_path) + '/' + os.listdir(recons_path)[0] + '/config.py'
    rec_config = load_config(recons_config_path)

    mask = load_mask(rec_config, proc_config, session)
    mask = np.where(mask >= rec_config.mask_eval_th, 1, 0)
    mask = mask == 1
    mask = cut_exp_mask(mask, rec_config.mask_eval_th, window)
    recons = np.load(str(recons_path.parent) + '/whole_session_recons/video_array.npy')
    recons_time = np.load(str(recons_path.parent) + '/whole_session_recons/video_timeline.npy')

    temporal_corr_new_evo = np.zeros(recons_time.shape)
    temporal_autocorr_evo = np.zeros(recons_time.shape)
    spectral_slope_new_evo = np.zeros(recons_time.shape)
    entropy_evo = np.zeros(recons_time.shape)
    compression_gain_new_evo = np.zeros(recons_time.shape)
    pca_energy_evo = np.zeros(recons_time.shape)
    frame_predictab_evo = np.zeros(recons_time.shape)
    temp_diff_energy_evo = np.zeros(recons_time.shape)

    print("Computing metrics")
    for i in range(len(recons_time) - window):
        print(f"{((i/len(recons_time)) * 100):.3f}" + "%")
        window_recons = recons[i:i + window]
        temporal_corr_new_evo[i + int(window / 2)] = temporal_correlation(window_recons, mask)
        temporal_autocorr_evo[i + int(window / 2)] = temporal_autocorrelation(window_recons, mask)
        spectral_slope_new_evo[i + int(window / 2)] = spectral_slope_new(window_recons, mask)
        entropy_evo[i + int(window / 2)] = frame_entropy(window_recons, mask)
        compression_gain_new_evo[i + int(window / 2)] = compression_ratio(window_recons, mask)
        pca_energy_evo[i + int(window / 2)] = pca_energy_ratio(window_recons, mask)
        frame_predictab_evo[i + int(window / 2)] = frame_predictability(window_recons, mask)
        temp_diff_energy_evo[i + int(window / 2)] = temporal_difference_energy(window_recons, mask)

    temporal_corr_new_evo = fill_edge_zeros_with_mean(temporal_corr_new_evo)
    temporal_autocorr_evo = fill_edge_zeros_with_mean(temporal_autocorr_evo)
    spectral_slope_new_evo = fill_edge_zeros_with_mean(spectral_slope_new_evo)
    entropy_evo = fill_edge_zeros_with_mean(entropy_evo)
    compression_gain_new_evo = fill_edge_zeros_with_mean(compression_gain_new_evo)
    pca_energy_evo = fill_edge_zeros_with_mean(pca_energy_evo)
    frame_predictab_evo = fill_edge_zeros_with_mean(frame_predictab_evo)
    temp_diff_energy_evo = fill_edge_zeros_with_mean(temp_diff_energy_evo)

    temporal_corr_new_evo = np.transpose(np.vstack((recons_time, temporal_corr_new_evo)))
    temporal_autocorr_evo = np.transpose(np.vstack((recons_time, temporal_autocorr_evo)))
    spectral_slope_new_evo = np.transpose(np.vstack((recons_time, spectral_slope_new_evo)))
    entropy_evo = np.transpose(np.vstack((recons_time, entropy_evo)))
    compression_gain_new_evo = np.transpose(np.vstack((recons_time, compression_gain_new_evo)))
    pca_energy_evo = np.transpose(np.vstack((recons_time, pca_energy_evo)))
    frame_predictab_evo = np.transpose(np.vstack((recons_time, frame_predictab_evo)))
    temp_diff_energy_evo = np.transpose(np.vstack((recons_time, temp_diff_energy_evo)))

    save_path = recons_path.parent / Path('metrics')
    os.makedirs(save_path, exist_ok=True)
    np.save(str(save_path) + '/temporal_corr_new.npy', temporal_corr_new_evo)
    np.save(str(save_path) + '/temporal_autocorr.npy', temporal_autocorr_evo)
    np.save(str(save_path) + '/spectral_slope_new.npy', spectral_slope_new_evo)
    np.save(str(save_path) + '/entropy.npy', entropy_evo)
    np.save(str(save_path) + '/compression_gain_new.npy', compression_gain_new_evo)
    np.save(str(save_path) + '/pca_energy.npy', pca_energy_evo)
    np.save(str(save_path) + '/frame_predictab.npy', frame_predictab_evo)
    np.save(str(save_path) + '/temp_diff_energy.npy', temp_diff_energy_evo)



if __name__ == '__main__':
    recons_run = '173'
    metric_window_t = 10    # seconds
    compute_recons_metrics(recons_run, metric_window_t)
