...
def get_optimizer_dict():
    optimizer_dict = {}
    # Iterate over all classes in the torch.optim module
    for name, obj in inspect.getmembers(optim, inspect.isclass):
        # Check if the class is a subclass of Optimizer and not the base Optimizer itself
        if issubclass(obj, optim.Optimizer) and obj is not optim.Optimizer:
            optimizer_dict[name] = obj
...
def ddp_setup(rank, world_size):
    ...
    init_process_group(
        backend="nccl",           # 'nccl' is usually best for GPUs
...
def train_mouse(rank, world_size, save_dir: Path, train_splits: list[str], val_splits: list[str]):
    ...
    optimizer = build_optimizer(model.nn_module, get_optimizer_dict(), argus_params['optimizer'])

    print("Aplicando DataParallel...")
    print("Using " +  str(world_size) + " GPUs!")
    
    model.nn_module.to(device=rank)
    model = DDP(model.nn_module, device_ids=[rank])

    if config.get("init_weights", False):
        print("Iniciando inicialización de pesos...")
        init_weights(model.module)

    if config.get("ema_decay", False):
        print(f"Configurando EMA decay: {config['ema_decay']}")
        model.module.model_ema = ModelEma(model.module, decay=config["ema_decay"])
    ...
    for num_epochs, stage in zip(config["num_epochs"], config["stages"]):
        ...
        for tr_batch, target in train_loader:
            memory_us
...
if __name__ == "__main__":
    ...
    for fold_split in folds_splits:
        ...
        mp.spawn(
            train_mouse, 
            args=(world_size, fold_experiment_dir, train_folds_splits, val_folds_splits),
            nprocs=world_size)
