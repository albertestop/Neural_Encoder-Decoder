...
for i, model_index in enumerate(model_list):
    ...
    for mouse_index in animals:
        ...
        for trial in trials_to_process:
            ...
            proc_params['video_params']['pixel_supression'] = 0
            proc_params['behavior_params']['speed'] = 'max'
            _, video, behavior, pupil_center, responses, population_mask, video_length = load_trial_data(
                trial_data_all, model, device, trial_index, proc_params, length=video_length)

            responses_predicted_no_speed = torch.zeros((constants.num_neurons[mouse_index], video_length), device=device)
            prediction = predictor[i].predict_trial(
                video=video,
                behavior=behavior,
                pupil_center=pupil_center,
            ...
            responses_predicted_no_speed[:, :] = torch.from_numpy(prediction).to(device)


            proc_params['behavior_params']['speed'] = 'original'
            ...
            response_corr_no_speed = utils.compute_corr(
                responses.cpu().detach().numpy(),
                responses_predicted_no_speed.cpu().detach().numpy()
            )
            ...
            axs[1, 1].imshow(responses_predicted_no_speed.cpu().detach().numpy(), aspect='auto', vmin=0, vmax=10)
            ...
            with open(trial_save_path + '/results.txt', "w") as file:
                ...
                file.write("\nGT Response correlation with response predictions made with max value of speed data: " + str(response_corr_no_speed))
                ...
                file.write("\nResponse predictions made with max value of speed data correlation with response predictions made with " + str(proc_params['video_params']['pixel_supression']) + " missing pixel: " + str(no_pxl_no_eye_corr))
            file.close()

            mean_response_corr.append(response_corr)
            mean_response_corr_no_eye.append(response_corr_no_speed) 
    ...
    with open(model_save_path + '/results.txt', "w") as file:
        ...
        file.write("\nGT Response correlation with response predictions made with max value of speed data: " + str(np.array(mean_response_corr_no_eye).mean()))
        ...
        file.write("\nResponse predictions made with max value of speed data correlation with response predictions made with " + str(proc_params['video_params']['pixel_supression']) + " missing pixel: " + str(np.array(mean_no_pxl_no_eye_corr).mean()))
    file.close()
